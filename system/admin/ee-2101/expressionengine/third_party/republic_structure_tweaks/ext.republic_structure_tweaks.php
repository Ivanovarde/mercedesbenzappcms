        ...
        $javascript .= <<<EOJS
            ...
            if (showStatus !== 'none') {
                if (showStatus === 'closed') {
                    $(".status-closed").find('.page-title:first').append('<span style="font-weight: normal; padding-left: 5px; color: #ab7780">(' + textClosed + ')</span>');
                }
                if (showStatus === 'not_open') {
                    for (var i = 0; i < statuses.length; i++) {
                        var status = statuses[i];
                        if (status.status !== 'open') {
                            $(".status-" + status.status).find('.page-title:first').append('<span style="font-weight: normal; padding-left: 5px; color: #' + status.highlight + '">(' + status.status_name + ')</span>');
                        }
                    }
                }
                if (showStatus === 'all') {
                    for (var i = 0; i < statuses.length; i++) {
                        var status = statuses[i];
                        $(".status-" + status.status).find('.page-title:first').append('<span style="font-size: 0.9em; font-weight: normal; padding-left: 5px; color: #' + status.highlight + '">(' + status.status_name + ')</span>');
                    }
                }
            }
...
EOJS;
    ...
    public function fetch_entries()
    {
        ...
        $this->entries = $this->_get_parent_fields();
    }

    private function _get_parent_fields()
    {
