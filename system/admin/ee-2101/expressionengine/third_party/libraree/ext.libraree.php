		...
		if($this->ENABLED){
			...
			if(isset($settings["path"])){
				...
				if(isset($settings['user_email_parsing_enabled'])){
					...
					//Try to reinitialize the Email Library with HTML mailtype
					$config = array(
						'protocol'		=> ee()->config->item('mail_protocol'),
...
{	
	...
	function fetch_email_template($name)
	{
		...
		if ($query->num_rows() == 0)
		{
			...
			$template_data_final = $this->EE->TMPL->parse_globals($this->EE->TMPL->final_template);	
