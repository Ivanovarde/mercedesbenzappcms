...
class Low_search_filter_keywords extends Low_search_filter {
	...
	public function filter($entry_ids)
	{
		...
		// Always this
		$t = ee()->low_search_index_model->table();
		...
		if (($match = $this->params->get('keywords:match')) &&
			($field = $this->fields->name($match)))
		{
			$join = $this->fields->is_native($match) ? 'channel_titles' : 'channel_data';
			$select[] = "IF({$field} = '{$keywords}', 1, 0) AS `match`";
		}
		...
		if ($join)
		{
			ee()->db->join($join, "{$t}.entry_id = {$join}.entry_id");
		}
