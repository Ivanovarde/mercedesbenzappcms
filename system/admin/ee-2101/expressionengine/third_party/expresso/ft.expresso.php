...
class Expresso_ft extends EE_Fieldtype {
	...
	function display_field($data)
	{
		...
		$this->EE->cp->add_to_head('<link rel="stylesheet" type="text/css" href="'.URL_THIRD_THEMES.'expresso/css/editor.css?v='.EXPRESSO_VERSION.'" />');
	...
	function grid_display_field($data)
	{
		...
		{
			$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.URL_THIRD_THEMES.'expresso/javascript/grid.js?v='.EXPRESSO_VERSION.'"></script>');
	...
	function display_cell($data)
	{
		...
		{
			$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.URL_THIRD_THEMES.'expresso/javascript/matrix.js?v='.EXPRESSO_VERSION.'"></script>');
	...
	function display_element($data)
	{
		...
		{
			$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.URL_THIRD_THEMES.'expresso/javascript/content_element.js?v='.EXPRESSO_VERSION.'"></script>');
	...
	private function _include_scripts($file_uploads)
	{
		...
		if (!isset($this->EE->session->cache['expresso']['included_scripts']))
		{
			...
			$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.URL_THIRD_THEMES.'expresso/ckeditor_4.4.7/ckeditor.js"></script>');
			$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.URL_THIRD_THEMES.'expresso/ckeditor_4.4.7/adapters/jquery.js"></script>');
			$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.URL_THIRD_THEMES.'expresso/javascript/expresso.js?v='.EXPRESSO_VERSION.'"></script>');
	...
	private function _get_config($id)
	{
		...
		// IVANO
		// Lists toolbar
		else if ($settings['toolbar'] == 'lists' && isset($settings['lists_toolbar']))
		{
			$js .= 'toolbar: ['.$settings['lists_toolbar'].']';
		}
		else if ($settings['toolbar'] == 'text' && isset($settings['text_toolbar']))
		{
			$js .= 'toolbar: ['.$settings['text_toolbar'].']';
		}
		// IVANO
	...
	*/
	function replace_element_tag($data, $params = array(), $tagdata = '')
	{
		...
		$tagdata = $this->EE->TMPL->parse_variables($tagdata, array(
			array(
				'value' => $data,
	...
	function display_global_settings()
	{
		...
		$this->EE->cp->add_to_head('<link rel="stylesheet" type="text/css" href="'.URL_THIRD_THEMES.'expresso/css/settings.css?v='.EXPRESSO_VERSION.'" />');
		$this->EE->cp->add_to_head('<link rel="stylesheet" type="text/css" href="'.URL_THIRD_THEMES.'expresso/ckeditor_4.4.7/skins/moono/editor.css?v='.EXPRESSO_VERSION.'" />');
		...
		foreach ($this->default_settings as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case 'toolbar_icons':
					...
					{
						$vars['settings'][$key] .= '<div class="icon_set">'.form_checkbox('toolbar_icons[]', $icon, in_array($icon, $val)).' <span class="cke_button_icon cke_button__'.str_replace(' ', '_', strtolower($icon)).'_icon" style="background-image: url('.URL_THIRD_THEMES.'expresso/ckeditor_4.4.7/skins/moono/icons.png);" title="'.$icon.'"></span></div>';
					}
				...
				case 'headers':
					...
					for ($i = 1; $i <= 6; $i++)
					{
						$vars['settings'][$key] .= '<div class="icon_set">'.form_checkbox('headers[]', 'h'.$i, in_array('h'.$i, $val)).'  <span class="cke_button_icon" style="background-image: url('.URL_THIRD_THEMES.'expresso/ckeditor_4.4.7/plugins/headers/images/h'.$i.'.png);" title="H'.$i.'"></span></div>';
					}
					$vars['settings'][$key] .= '</div>';
					break;
