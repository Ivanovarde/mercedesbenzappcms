<?php if ( empty($settings['google_token'])): ?>
	...
	<ul>
		...
		<li>If needed, create a project and name your project "Republic Analytics" (or something else)</li>
		<li>Click on "APIs & auth" and choose "APIs"</li>
		<li>Now a list of APIs should appear. You want to find "Analytics API" and switch that API to "ON"</li>
		<li>Choose "Consent screen" and make sure that the "Email Address" and "Product Name" are filled in.</li>
		<li>Choose "Credentials".</li>
		<li>Press <em>Create new Client ID</em> under OAuth to create your Client ID.</li>
		<li>Select <em>Web application</em></li>
		<li>Enter your EE installs domain name in "Authorized JavaScript Origins"</li>
		<li>
			Paste your Redirect URI in <em>Authorized Redirect URI</em>:
			<br />
			<code><?php echo $return_url;?></code>
			<br /><br />
			<em>
				The domain in <em>Authorized Redirect URI</em> must match the domain that you will be using Republic Analytics from. The only supported local domain
	...
	<p>
		<em>If it might take a few minutes for google to catch up, so try again in few minutes if it fails the first time.</em>
	</p>
	<p>&nbsp;</p>
...
<table class="mainTable padTable" cellspacing="0" cellpadding="0" border="0">
	...
	<tbody>
		...
		<tr class="even">
			...
			<td><input id="visits_color" type="text" name="visits_color" value="<?php echo set_value('visitors_color', $settings['visits_color']);?>" autocomplete="off" style="width: 90%; float: left; background-image: none; text-shadow: none;"/></td>
		</tr>

		<tr class="odd">
			<td><label for="visitors_color"><?php echo lang('republic_analytics_configuration_visitors_color');?></label></td>
			<td><input id="visitors_color" type="text" name="visitors_color" value="<?php echo set_value('visitors_color', $settings['visitors_color']);?>" autocomplete="off" style="width: 90%; float: left; background-image: none; text-shadow: none;" /></td>
		</tr>
		<tr class="even">
		...
		<tr class="odd">
			...
			<td><input id="pages_view_color" type="text" name="pages_view_color" value="<?php echo set_value('pages_view_color', $settings['pages_view_color']);?>" autocomplete="off" style="width: 90%; float: left; background-image: none; text-shadow: none;" /></td>
		</tr>
	</tbody>
</table>
