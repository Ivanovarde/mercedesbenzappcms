...
class Calendar_datetime extends Calendar
{
	...
	public function format_date_character($format)
	{
		...
		{
			// -------------------------------------
			//	This is a hack needed due to the fact
			//	that event dates are stored as y-m-d-hour
			//	instead of unix timestamps. This design
			//	decision affects the entire addon and
			//	is not something I would have done but
			//	changing it would require a complete rewrite.
			//	This just does the best we can to get
			//	the output correct.
			//
			//	Entry_id and calendar_id are set outside
			//	of the class on the instance. If they
			//	are missing we have to give bad output.
			// -------------------------------------

			$offset = 0;

			//do this first so we don't double code the calendar
			//lookup
			if (isset($this->entry_id) && ! isset($this->calendar_id))
			{
				$entry_cal_query = ee()->db
									->select('calendar_id')
									->where('entry_id', $entry_id)
									->get('exp_calendar_events');

				if ($entry_cal_query->num_rows() > 0)
				{
					$this->calendar_id = $entry_cal_query->row('calendar_id');
				}
			}
			...
			if (isset($this->calendar_id))
			{
				...
				if (isset($this->cache['cal_offsets'][$calendar_id]['offset']))
				{
					$offset = $this->cache['cal_offsets'][$calendar_id]['offset'];
				}
				else
				{
					$cal_fmt_query = ee()->db
									...
									->get('exp_calendar_calendars');

					if ($cal_fmt_query->num_rows() > 0)
					{
						$offset = $cal_fmt_query->row('tz_offset');
						$this->cache['cal_offsets'][$calendar_id] = array();
						$this->cache['cal_offsets'][$calendar_id]['offset'] = $offset;
					}
					else
					{
						$offset = $this->cache['cal_offsets'][$calendar_id] = 0;
					}
				}

				// -------------------------------------
				//	Datetime available? Allow override
				//	with timezone
				// -------------------------------------

				$has_tz_override = false;

				foreach (ee()->TMPL->tagparams as $key => $value)
				{
					if (preg_match("/^tz_id_/", $key))
					{
						$has_tz_override = true;
					}
				}

				//Someones poopy install might not
				//have the built in classes for some
				//weird reason.
					...
					$has_tz_override)
				{
					...
					if (ee()->TMPL->fetch_param('tz_id_' . $calendar_id) !== FALSE)
					{
						$tz_id = ee()->TMPL->fetch_param('tz_id_' . $calendar_id);
					}

					if ( ! $tz_id)
					{
						$calendar_url_title = false;

						if (isset($this->cache['cal_offsets'][$calendar_id]['url_title']))
						{
							$calendar_url_title = $this->cache['cal_offsets'][$calendar_id]['url_title'];
						}
						else
						{
							$calendar_title_query = ee()->db
								->select('url_title')
								->where('entry_id', $calendar_id)
								->get('channel_titles');

							if ($calendar_title_query->num_rows() > 0)
							{
								$this->cache['cal_offsets'][$calendar_id]['url_title'] = $calendar_title_query->row('url_title');
								$calendar_url_title = $this->cache['cal_offsets'][$calendar_id]['url_title'];
							}
						}

						if ($calendar_url_title &&
							ee()->TMPL->fetch_param('tz_id_' . $calendar_url_title) !== FALSE)
						{
							$tz_id = ee()->TMPL->fetch_param('tz_id_' . $calendar_url_title);
						}
					}
					//END if ( ! $tz_id)
					...
					if ($tz_id)
					{
						...
						//Wrapping in a try/catch in case they input
						//an incorrect timezone id
						try {

							$date = new DateTime(
								$time_string,
								//new DateTimeZone('America/New_York')
								new DateTimeZone($tz_id)
							);
						...
						{
							//if this were a more prominant feature
							//we should notify the user of the failure here
							//but since this is a bandaid on a bandaid we
							//are going to fallthrough to previous behavior
							//for now. A rewrite of this class, if it ever
							//happens will remove the need for all of this
							//anyway.
						}
					}
					//END if ($tz_id)
				}
				//END if (class_exists('DateTime')
			}
			//END if (isset($this->calendar_id))

			if ($output == '')
			{
				if ($offset > 0)
				{
					//turns offset -0500 into -18000 seconds
