...
class Calendar_ics_field_update
{
	 ...
	 * @param	array	$entry_id	optional entry id needed for some fields
	 * @return	array	row with fields prepped for saving
	 */

	public function run_prep_fields($row = array(), $entry_id = 0)
	{
		...
		foreach ($row as $field_name => $data)
		{
				...
				in_array(self::$field_type_cache[$match[1]], $this->special_fields))
			{
				...
				{
					$n_data = $this->{$type . '_prep_data'}($data, $entry_id);
	...
	public function assets_prep_data($data = '', $entry_id = 0)
	{
