		...
		*/
		...
		if (preg_match(
				"/" . LD . "calendar_paginate" . RD . "(.+?)" .
					  LD . preg_quote($this->t_slash, '/') . "calendar_paginate" . RD . "/s",
				ee()->TMPL->tagdata	,
				$match
			))
		{
...
ee()->TMPL->log_item('Calendar: Channel module says no results, bailing');
		...
		foreach ($channel->query->result as $k => $row)
		{
			...
			$this->set_cdt_data($entry_id, $events);
...
ee()->TMPL->log_item('Calendar: Weblog query is empty, bailing');
			...
			$this->event_timeframe_total > $this->P->value('event_limit'))
		{
			...
			if ($pagination_data['paginate'] === TRUE)
			{
				...
				$this->pagination_array	= $pagination_data['pagination_array'];
...
ee()->TMPL->log_item('Calendar: Processing parameters');
	...
	*/
	...
	public function ics_update()
	{
		...
		$force_update = $this->check_yes(ee()->TMPL->fetch_param('force_update'));
		$allow_logged_out = $this->check_yes(ee()->TMPL->fetch_param('allow_logged_out'));

		foreach ($ids as $id)
		{
			$this->actions->import_ics_data($id, false, $force_update, $allow_logged_out);
		}
	}
	/* END ics_update() */
...
ee()->TMPL->log_item('Calendar: Output time');
	...
	protected function build_calendar()
	{
		...
		{
			$this->paginate_tagpair_data	= $match[0];
			ee()->TMPL->tagdata = $tagdata	= str_replace( $match[0], '', $tagdata );
		}
		//prefix comes first
		else if (preg_match(
				"/" . LD . "paginate" . RD .
...
ee()->TMPL->log_item('Calendar: Outputting year data for '.$ymd);
	...
	{
		$this->pagination_links = $this->pager;

		if (stristr($this->paginate_data, LD . 'calendar_pag') !== false)
		{
			return parent::parse_pagination(array(
				'prefix'		=> 'calendar_',
				'tagdata'		=> $return,
				'auto_paginate'	=> true
			));
	...
	protected function get_occurrence_vars($id, $event, $ymd, $time = '00000000')
	{
		...
		$this->set_cdt_data($id, array($id => $event));
	...
	protected function set_cdt_data($entry_id, $event_array)
	{
		$this->CDT->entry_id = $entry_id;

		if (isset($event_array[$entry_id]->default_data['calendar_id']))
		{
			$this->CDT->calendar_id = $event_array[$entry_id]->default_data['calendar_id'];
		}
	}
}
// END CLASS Calendar