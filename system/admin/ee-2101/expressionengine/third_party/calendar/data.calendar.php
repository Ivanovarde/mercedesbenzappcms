...
class Calendar_data extends Addon_builder_data_calendar
{
	...
	public function get_module_preferences($refresh = FALSE)
	{
		...
		$upd = new Calendar_upd();

		$prefs 	= $upd->_default_preferences();
	...
	public function get_calendars_needing_update($ids, $minutes, $status = 'open')
	{
		...
		$sql = "SELECT 		cc.calendar_id
				...
				IN 			($statuses)
				AND 		cc.ics_url != ''
				AND 		cc.ics_updated < FROM_UNIXTIME(" .
								//we add 30 seconds to 'now' just as a failsafe because
								//a slight error in timing could cause a 5 minute cron
								//+ 5 minute interval for update to only update every
								//10 minutes due to the math matching exactly to 5 minutes.
								//----------!!!!!!!!!!!----------
								//DO NOT USE MYSQL UNIX TIME HERE BECAUSE WE NEED
								//A STATIC TIME FOR ALL UPDATE CHECKS IN CASE ONE TAKES
								//LONGER THAN ANOTHER DURING A MASS UPDATE
								ee()->db->escape_str((ee()->localize->now + 30) - ($minutes * 60)) . ")";
	...
	public function update_ics_updated($calendar_id)
	{
		...
		ee()->db->query(
			'UPDATE exp_calendar_calendars
			 SET 	ics_updated = FROM_UNIXTIME(' . ee()->db->escape_str(ee()->localize->now) . ')
			 WHERE 	calendar_id = '. ee()->db->escape_str($calendar_id)
		);
	}
	...
	public function fetch_event_ids($params, $category = FALSE)
	{
		...
		$cache_name = __FUNCTION__;
		$cache_hash = $this->_imploder(array($params->params, $category));
