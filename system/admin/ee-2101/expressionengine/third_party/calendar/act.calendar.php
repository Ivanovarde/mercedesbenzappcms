...
class Calendar_actions extends Addon_builder_calendar
{
	 ...
	 * Import ICS Data
	 *
	 * Import (and update) events from ICS url field in calendars
	 *
	 * @access	public
	 * @param	integer		$calendar_id		calendar id to update
	 * @param	boolean		$run_deletes		run deletions for missing items?
	 * @param	boolean		$force_update		ignore date change checking for events
	 * @param	boolean		$allow_logged_out	use hacks for channel entries API logged out
	 * @return	boolean							success (perceived)
	 */

	public function import_ics_data (
		$calendar_id,
		$run_deletes = false,
		$force_update = false,
	...
	)
	{
		...
		// -------------------------------------
		//	allow logged out updates?
		// -------------------------------------

		if ($allow_logged_out)
		{
			$author_q = ee()->db->select('group_id')
								->where('member_id', $author_id)
								->get('members');

			$old_group_id = ee()->session->userdata['group_id'];

			if ($author_q->num_rows() > 0)
			{
				ee()->session->userdata['group_id'] = $author_q->row('group_id');
			}

			ee()->session->userdata['group_id'] = 1;

			$old_can_edit = ee()->session->userdata['can_edit_other_entries'];
		...
		foreach ($urls as $url)
		{
			...
			while ($event = $ICAL->getComponent('vevent'))
			{
				...
				$_POST = $post_data = array(
					...
					//MANUAL EDIT to not have entries as open
					'status'					=> 'Pending',
				...
				if ($entry_id)
				{
					...
					if ($old_data_q->num_rows() > 0)
					{
						...
						$old_data = ee()->calendar_ics_field_update
											->run_prep_fields($old_data, $entry_id);
		...
		if ($allow_logged_out)
		{
			ee()->session->userdata['group_id'] = $old_group_id;
			ee()->session->userdata['can_edit_other_entries'] = $old_can_edit;
		}
