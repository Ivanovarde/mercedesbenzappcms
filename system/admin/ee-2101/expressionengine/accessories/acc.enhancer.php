...
class Enhancer_acc {
    ...
    function set_sections()
    {
       // $this->EE->cp->load_package_js('hosei');
        ...
        $this->EE->cp->add_to_foot('<script type="text/javascript" charset="utf-8">$(document).ready(function() {$(\'.hosei\').parent().remove();});</script><style type="text/css" media="screen">.mainTable tr:hover > td{background-color: #e3e7e9 !important;}</style>');
			
        if($module == 'comment') {
        	$this->EE->cp->add_to_foot('<style type="text/css" media="screen">.tableSubmit{ clear:both; }</style>');
			$this->EE->cp->load_package_js('comments');
        }
		
//		if($C == 'homepage') $this->EE->cp->add_to_foot('<script type="text/javascript" charset="utf-8">
...
//			});</script>');
        ...
        if($C == 'addons_modules' && $M == false){
            $this->EE->cp->add_to_foot(str_replace("\n",'','<script type="text/javascript" charset="utf-8">
				$(document).ready(function() {
		...
		if($C == 'addons_extensions' && $M == false){
            $this->EE->cp->add_to_foot('<script type="text/javascript" charset="utf-8">
				$(document).ready(function() {
