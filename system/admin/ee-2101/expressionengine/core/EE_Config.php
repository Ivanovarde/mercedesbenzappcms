...
class EE_Config Extends CI_Config {
	...
	function site_prefs($site_name, $site_id = 1)
	{
		...
		{
			// IVANO
            $site_name = $this->default_ini['site_name']; 
            //$site_name = '';
            // IVANO
			$site_id = 1;
...
//        if ($site_id == 1)
//		{
//            $sql = "SELECT es.* FROM exp_sites AS es WHERE es.site_id = '".$DB->escape_str($site_id)."'";
//			$query = $DB->query($sql);
//		}
//        
//
...
//		}
        ...
        //$this->config['site_label'] = $this->default_ini['site_label'];
        $this->config['site_short_name'] = $this->default_ini['site_name'];
		$this->config['site_name'] 		 = $this->default_ini['site_label']; // Legacy code as 3rd Party modules likely use it
