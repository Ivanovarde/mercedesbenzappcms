...
<div id="footer">
	Version 
	<?=(IS_CORE)?' Core':''?> v<?=APP_VER?> <?php echo ' - ';
	echo lang('build'). '&nbsp;'.APP_BUILD;?> - &copy; <?=lang('copyright')?> 2003 - <?= date('Y') ?><br />

	<?php
		if(valid_license_pattern(config_item('license_number'))){
			echo config_item('license_number') . ': ' . $license . ' - ' . mailto(config_item('license_contact')) . '<br>';
		}
	?>
...
<div id="idle-modal" class="pageContents">
	...
	<div class="idle-three-fourths shun">
		<p class="idle-fourth">
            ...
            <?PHP
            $iv_avatar = $this->session->userdata('group_id') == 1 ? 'avatar_default_superadmin' : ($this->session->userdata('group_id') == 6 ? 'avatar_default_admin' : 'avatar_default_member');
            ?>
            <img src="<?=config_item($iv_avatar) ? config_item($iv_avatar) : ($cp_avatar_path ? $cp_avatar_path : $cp_theme_url.'images/site_logo.gif')?>" width="50" alt="<?=lang('user_avatar_alt')?>" />
            <!-- IVANO -->
		</p>
