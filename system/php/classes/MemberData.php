...
class MemberData extends ABM {
	...
	function __construct($id=""){
		...
		$this->readDBFields(Tables::MEMBERDATA);
		$this->read($id);

		if($id==''){
			$this->priKeyField = "";
		}else{
			$this->priKeyField = "member_id";
		}

	}

	private static function getMemberData($sql){
		$db = new DB();
		$db->setQuery($sql);
		$record = new MemberData($db->executeValue());
		return $record;
	}

	private static function getMemberDatas($sql) {
		$db = new DB();
		$db->setQuery($sql);
		...
		Log::l('Section getMemberDatas', $vAll, false);
		return $a_elements;
	}
